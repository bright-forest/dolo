"""SymbolicModel — the core syntactic stage object.

Moved from dolo.compiler.model (spec 0.1r).
"""
...
def sym(raw_dict):
    """Parse raw YAML dict into SymbolicModel."""
    import yaml
    if isinstance(raw_dict, dict):
        yaml_text = raw_dict.get("yaml_text", "")
        yaml_path = raw_dict.get("yaml_path", "<string>")
        data = yaml.compose(yaml_text)
        return SymbolicModel(data, filename=yaml_path)
    return SymbolicModel(raw_dict)
